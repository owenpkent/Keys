...
| **Settings** | gear → menu | Immediately left of Theme, plugin-level like the swatch so it never hides with the Controls fold (2026-08-17, Owen: "we need a settings icon and menu. populate menu"). Four groups: **Hold visuals during sustain** (default on, and today's behaviour either way - off makes a key held only by the pedal rest visually while it keeps sounding, a paint-only difference); **Sustained drag leaves a trail** (default on, also today's behaviour - whether a glide made with the pedal down piles up every key it crosses or stays monophonic; the name is not Octavium's "Drag While Sustain", since Keys' drag has always glided and that toggle would have promised something no switch here can do); **Sustained notes propose chords** (default **off** - see the Chord pads section below); **Reset all settings...** in a group of its own (2026-08-27, Owen: "we need reset button in settings"), the only row here that changes what you hear: it puts every parameter back to its default, along with the three switches in this same menu and **Light keys**, **Play** and **Keep arp** (behaviour that lives in the layout tree rather than as a parameter - a row named "Reset all settings" that left the settings six pixels above it untouched would be wrong where its name is read). It stops there: your chord pads, arp patterns and slots are left exactly as they are, because those are work you made and undo already covers them, and so are your theme, which sections are open, where the detached windows sit and your library favourites. It asks first, and that confirmation is the way back rather than a formality, since undo covers content and not parameters. It releases every sounding note before it runs. Then **Check for updates** / **User guide** / **About**. **About** names the plugin's **MCP port** as of 2026-08-27, and the host track it is loaded on when the DAW reports one (Ableton Live 12 does). That is the only thing on screen that tells one instance from another: several Keys in one set share a process, so nothing in the DAW distinguishes them, and a script driving Keys over MCP addresses them by that port. *Chord pads play while held* was a fifth item here from 2026-08-18 and was removed on 2026-08-22, when the Pads bar's **Play** toggle became that behaviour outright. |
...
| **Track MIDI** | toggle, arp bar | **Off by default** (2026-08-27). Off: the arpeggiator ignores MIDI arriving on this track, so a clip or another device cannot start it — the notes still reach your instrument untouched, since this decides what the *arpeggiator* hears and not what the track plays. On: a clip on this track drives the arp, which is how you feed it a progression the host holds the timing of. Global rather than per line, for Scale Lock's own reasoning: it is one door into the instance, and a door shut for A and open for C is not shut. Per-line **PLAY** is the separate question of whether the *keyboard you click* feeds that line, and it still defaults on. The two were one switch until this existed, which is why six untouched instances in one set could all be listening to the track and four of them start arpeggiating the moment you pressed record. A **dot on that line's letter** (A/B/C/D) means it is sounding notes nobody handed it — the on-screen half of `get_state`'s `soundingNoteCount`. Holding keys on the keybed counts as handing it something, so the dot stays dark while you play a line whose **PLAY** is on: it is there for the case where nothing on screen explains the noise. |
...
| **Legato** | on the same strip (2026-09-01). Holds a note through the steps that do not fire - Density turned down, a Chance-lane cell, a mute, a rest, a failed Chain - and lets it go one sample *after* the next note that does, so a synth in legato or glide mode slides across the gap instead of restarting. A note whose next step fires still ends at its gate; only the gaps are filled (the engine looks one step ahead to know which). A ratchet holds only its last sub-hit. Off by default, and off is exactly what the arp did before: a skipped step is silence. Card-only, like Mutate, Stray and Lock |
| **From** | on the same strip (2026-09-01, the line bus): which line this one listens to - Off, or a letter *above* it, so signal flows A to D and nothing can loop. The letters at or below this line are greyed in the popup rather than missing; line A opens to Off and three reasons why. What the line does with what it hears: **Duck** and **Clock** on the knob strip, **Follow** in the Play page's Retrigger list (restart when that line comes round), and `3` / `4` on the Draw page's Chain lane (play only with, or only against, that line's last step). A source that is off or silent leaves this line playing as if it followed nobody - **Clock is the one exception**, below |
...
| **Density** | how many of its steps actually play (2026-09-01). At 100 every step fires; turn it down and steps drop out at random, thinning the run. It is the Play page's **Density** slider - Chance until that day - given a face where you sit and turn things, so the same parameter under one name on both surfaces; it multiplies the Chance *lane*, which keeps its own name because it is per-step odds rather than a whole line's. With Legato on, the note before a dropped step holds through it |
| **Duck** | how often this line skips a step right after the line it follows (**From**, on the strip below) played one - the hocket (2026-09-01). Where that line plays, this one leaves the gap: two Up runs on one chord become one interlocking part neither is playing. Exact across blocks, rolled on Mutate's cell so Lock holds what it finds, spends no chance draw, and the first step after From or Duck comes up never ducks. At 0, or with From off, nothing changes. Under Legato a ducked step is a gap when the note before it had a gate under 100, since the lookahead cannot see the source's next step |
...
| **Keybed** | after the dice (2026-09-01): does what you play on the keybed feed this line? The same switch as **Keybed** on the line's Play page (it read *Play* there until that day, and lived there alone - Owen found it off with nothing on screen to say so). Off, the line ignores the keyboard and plays only the chord cards you hand it. Track MIDI on the arp bar is the separate question of MIDI arriving on the track |
| **Clock** | after Keybed (2026-09-02): stop running this line's own clock and take one step for every step the line named in **From** plays, at the exact offset it plays it - swung, late, thinned by Density, cut by a Chance-lane cell, whatever the source did to get there. A source ratcheting three sub-hits on a step still counts as one; **Gate now reads against the source's step**, not a rate this line no longer has, so 50% still means half the space to the next note you hear. With Clock on, this line's own Rate, its `<` `>` steppers, Sync/Hz, Swing, Dot, Tuplet and Anchor all grey out here and on the Play page - there is no clock of this line's own left for them to touch - and the Cards page's rhythm dividers grey with them (a different **Clocks**, the four dividers under one line's own pattern - see Euclid and Clocks, below - since a divider divides a clock this line no longer runs). **Duck** still runs underneath: a clocked step always lands right on a source fire, so at Duck 100 every step after the first goes silent. A clocked line with nothing to clock to - From at Off, or naming a letter at or below this line - plays nothing at all, the one place a source that is off or silent does not simply leave the line be. Switching Clock off hands the line straight back to its own clock wherever it had got to, with no burst of the steps it missed. Worth trying: a swung, ducked A driving a shorter, differently-shaped B - one pulse wearing two patterns |
| the chord | what that line is holding, or `...` while a quantized launch is waiting |

Eleven knobs (ten for a few hours on 2026-09-01, before Duck joined Density; nine from 2026-08-21 to 2026-09-01, when Density - the Play page's Chance slider under its new name - came back onto the card between Gate and Mutate; eight between 2026-08-18, when Chance became Mutate and Lock, and 2026-08-21, when Stray moved out of Mutate's upper half onto a knob of its own; seven before that, from 2026-08-17, when Vel absorbed H.Vel's own knob - Chance is still a step lane), two Harmony dropdowns each with its own Chance knob (2026-08-19; the dropdown's list opens as two columns, minus intervals left and plus intervals right, the way BigSky's panel draws it), a rate with its three
modifiers, a shape with its dice, Dot/Tuplet/Anchor and Details: every setting a regular arpeggiator has, all
four lines deep, on one screen. Latch and Chain still exist per line, just not on this
card - Latch lives on that line's own Details view (the band), Chain on its action row (the keybed switch, **Keybed**, is on the card since 2026-09-01 and on the band as well)
...
| **Density** | Playback | 0–100%. How many of the line's steps actually play — turn it down to thin a run out. Works on any shape, and multiplies the Chance lane. **Chance until 2026-09-01**, when it grew a face on the macro card as DENSITY: one parameter, one word on both surfaces, and not the lane's word because the lane is per-step odds where this thins the whole line. |
| **Retrigger** | Playback | When the pattern starts over: **Off**, **Note** (a new chord restarts it), a clock window — 1 or 2 beats, 1, 2 or 4 bars — or **Follow** (2026-09-01): when the line this one follows (**From**, on its card) comes round to the top of its own walk. A clock window is what makes a five-step lane still land on the bar; Follow is what makes seven steps against another line's sixteen drift for a bar and snap home. Follow is a parameter of its own (`arpResetFollow`) behind the same list, and a silent source resets nobody. |
...
| Page | What is on it |
|------|----------------|
| **Play** | How the line plays: Rate, Shape, Tuplet/Dot, Swing, Gate, Density, Retrigger, Spread and Feel. Most of what you want is here. |
...
| **Chance** | 0-100% | Odds this step fires at all, multiplied by the **DENSITY** knob on the line's card and the Play page (called CHANCE there until 2026-09-01 - the knob thins the whole line, the lane is one step's odds, so they read differently on purpose). Called **Prob** until 2026-08-14; one word for one idea. |
...
| **Chain** | 0 to 4 | Play this step only on a condition. `0` always, `1` only if the step before it sounded, `2` only if it did not; **`3` only if the line this one follows just sounded, `4` only if it did not** (2026-09-01 - pick that line with **From** on the card; with nobody to follow, 3 and 4 play every step). For two lines on one rate "just sounded" is the same step, since that line runs first. Chance says *maybe*; this says *only if*. |
...
retitles to Chaining. **Not the same Clock as the macro card's chip** (2026-09-02): that one
takes this line's steps from another line entirely, and while it is on, this whole strip greys
- a divider divides a clock the line no longer runs. See the macro card's **Clock** row, above.
