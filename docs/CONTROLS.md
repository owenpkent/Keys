...
| **From** | on the same strip (2026-09-01, the line bus): which line this one listens to - Off, or a letter *above* it, so signal flows A to D and nothing can loop. The letters at or below this line are greyed in the popup rather than missing; line A opens to Off and three reasons why. What the line does with what it hears: **Duck** on the knob strip, **Follow** in the Play page's Retrigger list (restart when that line comes round), and `3` / `4` on the Draw page's Chain lane (play only with, or only against, that line's last step). Clock is designed in `docs/LINE_INTERACTION.md` and not built. A source that is off or silent leaves this line playing as if it followed nobody |
