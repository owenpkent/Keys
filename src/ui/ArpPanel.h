...
public:
    ...
    // The Select span, for the three views on the Draw page. LaneGrid draws it and, in Select
    // mode, writes it; the span belongs to the panel because Roll and Reset narrow to it and
    // both of those are the panel's own buttons. Four lines rather than a friend declaration:
    // when the grids lived inside this class they simply reached in, and this is the whole of
    // what they were reaching for (2026-09-02). The setter writes and nothing else - the caller
    // repaints, the way it always did, since a drag updates the span on every mouse move.
    bool selectModeOn() const { return selectMode; }
    int selectionFrom() const { return selFrom; }
    int selectionTo() const { return selTo; }
    void setSelectionSpan(int from, int to) { selFrom = from; selTo = to; }
    // Is Link on for the line being edited? LoopBar's one question of the panel, asked here
    // rather than by handing out paramId: Link on means the lanes share one grid, so a loop
    // window drag writes every lane.
    bool lanesLinked() const;
    ...
    // Spelled once in ArpRateMode.h, which MacroRow.h needs before this class exists;
    // re-exported here so ArpPanel::ComboAtt still names the same type.
    using ComboAtt = keys::ComboAtt;
    using SliderAtt = keys::SliderAtt;
    using ButtonAtt = keys::ButtonAtt;

    // MacroRow, LaneGrid, LoopBar and MuteRow are written in files of their own since
    // 2026-09-02: this one had grown to 4,687 lines with all four defined inline, and every one
    // of them is self-contained - a card and three step views that read the panel through its
    // public answers and write straight into the arp tree. They stay *named* here, as member
    // aliases, because a nested type is part of the interface whatever file it is written in:
    // LayoutTests finds the macro cards by `ArpPanel::MacroRow` and counts its knobs by
    // `ArpPanel::MacroRow::numKnobs`, and nothing about that had to move.
    //
    // See MacroRow.h for the card and the layout constants the All view measures itself with,
    // and LaneGrid.h for the three Draw-page views and the step geometry they share.
    using MacroRow = keys::MacroRow;
    using LaneGrid = keys::LaneGrid;
    using LoopBar = keys::LoopBar;
    using MuteRow = keys::MuteRow;
...
private:
    ...
    // The swap itself is arprate::applyMode (ArpRateMode.h), shared with every macro card.
