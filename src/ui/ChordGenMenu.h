...
// Chords view:
//
//   * the two constructive bulk actions, Fill and Regen, are chips on the **Pads section
//     bar** - a bar is 34 px that already exists, so controls riding it cost the window no
//     height at all. They are the only left-click path into generation, which is why they are
//     not on a menu. Clear page is not up there with them: see fillPage/clearPage below;
...
//
// The settings hang **directly off the pad menu**, one level down, not inside a "Generator
// settings" wrapper (2026-07-30, Owen's ask). That wrapper cost a third leg of diagonal hover
// travel - right-click, hover the wrapper, hover the setting, click the value - and hover
// travel is the expensive thing with one mouse. Two levels is the ceiling here. The Markov
// group is the one exception and stays a submenu of its own: five settings that do nothing
// until Source is Markov, kept together so the flattened menu still fits on a screen.
//
...
public:
    ...
    // The page-wide actions. fillPage is wired to the two chips on the Pads bar;
    // `onlyUnlocked` is Regen, which refreshes the pads that already carry a chord and leaves
    // the blanks alone. clearPage is reached from a pad's card menu instead: it wipes sixteen
    // pads, there is no undo in Keys, and a chip on a bar sat one slip away from the page
    // buttons. Constructive actions get the bar; destructive ones get the extra click.
    void fillPage(bool onlyUnlocked);
    void clearPage();

    // The generator's items on a pad's card menu, and what to do with a choice from them.
    // ChordPads builds the menu and calls these two, in that order, on the message thread.
...
private:
    ...
    // Every setting, added straight onto the menu it is handed rather than into a submenu of
    // its own. See the class comment: the wrapper it used to build was a whole extra level of
    // hover for nothing.
