...
| **Velocity** | A **range knob** in the pads strip, captioned Humanize. The knob is the hardest a note ever lands and the ring reaches back from it; the lamp beside it switches it. Lit, each note takes a random value inside the band; unlit, every note plays the band's midpoint and the readout shows that one number |
...
| **Humanize** | The lamp on the Velocity knob above: draw each note's velocity at random from its range, so repeats and chords don't sound machine-perfect. It had a tick box of its own until 2026-08-03 |
...
| **Undo / Redo** | Left end of the Controls bar, and reachable whatever is folded. They cover anything that destroys music - clearing, overwriting or moving a chord pad, Fill, Regen, Clear page, dragging a card off the strip, and on the arp side Roll, Reset, Randomize, Euclid, copying a slot, drawing a lane and muting steps - thirty-two deep, one entry per *gesture* rather than per change. Knobs are deliberately excluded: a knob you can always turn back, and if every dial sweep filled the stack the pad you actually wanted back would be pushed off the end of it |
...
| **Sync** | Tempo Sync (`bpmSync`, default on): the opt-out from following a playing host's tempo. Keys always followed the host before this toggle existed, so on reproduces that exactly; off pins the arp and the progression chain to BPM even while the host rolls. While Sync is on and a host tempo is actually live, the BPM field shows the host's own number and greys out |
| **Arp** | Its own section, and the one Keys opens showing. **Two arpeggiators, A and B**, each with its own rate, shape, pattern, twelve slots, chord and chain — so a polyrhythm is one plugin, not two. **A and B on the bar are each line's own On switch** (2026-08-02) as well as a chord-drop target, alongside **Hold off**, **All Off** (both lines off and everything let go, in one click) and **Light keys** (lights the keyboard for what the arp is playing — display only) — all of which survive the section being folded shut; only the **All** tab and the three page tabs beside them hide with the fold, since they just open the view below. Inside, the **All** view puts both lines on screen as **two boxed cards, side by side**: rate with its Sync/Hz switch and its Dot / Tuplet / Anchor, shape, eight knobs (Oct, Gate, Chance, Swing, Offset, Vel, H.Time, H.Vel — the last two are **range knobs**, where the knob sets the most the randomness ever does and a ring around it sets how far under that a hit can fall — drag the little dial at its top left, or anywhere on the ring, and turn the knob to move the whole range at once, so a line can be *always* a touch late and a touch softer with the variation on top), a **Details** button and the chord it holds, over **Launch Quantize**, which the two lines share (the tempo itself lives on the *Controls* bar, with a **Sync** toggle beside it to opt out of following the host). **Oct is a transpose, centred at zero**, so a line sits an octave under the other rather than only ever stacking upward. An off line's card is greyed out but every control on it still works, since a chord can still be dropped on it and a rate dialled in before switching it on. Each card's **Details** button goes deep on that line, which is **three pages** picked on the bar beside All: **Play** (rate, shape, swing, gate, chance, spread and feel - most of what you want), **Cards** (the twelve launchable slots, each holding a pattern, a chord and a rate that one click installs, plus Randomize, Euclid, Clocks and Chain) and **Draw** (the twelve-lane step editor). Paging is what stopped the window resizing: every page fits one fixed height, so opening a line no longer grows the window by 372 px and closing it no longer shrinks it back — A and B on the bar used to do this and are the On switch now instead. Hand a line a chord by **dragging** the card onto a slot, onto A or B, or onto a line's row in the All view; a click only ever plays the pad. **Handing a chord to a line that is off is silent and not lost** — the line holds it and starts the moment you switch it on. Twelve shapes (including **Chord**, which plays the whole held chord every step) plus **Pattern**; **Distance** stacks the chord by scale degrees rather than fixed intervals; **Chain** plays a line's twelve slots as a progression, each for the bars its card shows |
