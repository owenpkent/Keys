...
| **Arp** | Its own section, and the one Keys opens showing. **Two arpeggiators, A and B**, each with its own rate, shape, pattern, twelve slots, chord and chain — so a polyrhythm is one plugin, not two. The bar carries a switch per line, **Hold off**, **All Off** (both lines off and everything let go, in one click) and **Light keys** (lights the keyboard for what the arp is playing — display only), all of which survive the section being folded shut. Inside, the **All** view puts both lines on one screen as rows: rate with its Sync/Hz switch and its Dot / Trip / Anchor, shape, seven knobs (Oct, Gate, Chance, Swing, Offset, Vol, Human), the chord it holds and its Chain, over the tempo and **Launch Quantize** they share. **Oct is a transpose, centred at zero**, so a line sits an octave under the other rather than only ever stacking upward. The A and B tabs beside it go deep on one line: the full control band, the ten-lane step editor and twelve launchable slots, each holding a pattern, a chord and a rate that one click installs. Hand a line a chord by clicking a card with that line on, or by dragging the card onto a slot, a tab or a line's row. **Handing a chord to a line that is off is silent and not lost** — the line holds it and starts the moment you switch it on. Twelve shapes (including **Chord**, which plays the whole held chord every step) plus **Pattern**; **Distance** stacks the chord by scale degrees rather than fixed intervals; **Chain** plays a line's twelve slots as a progression, each for the bars its card shows |
...
| **Quantize** | In the macro view, shared by both lines. Off fires a chord the instant you click it; anything else (1/16 up to 2 bars) holds the click until the next boundary, so a card can only ever land on the grid. Ableton's Quantization, for the arp. It never delays the keys you play |
